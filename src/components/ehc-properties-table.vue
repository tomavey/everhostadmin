...
<template>
  <v-container fluid>
    ...
    <ehc-sheet>
      <ehc-table :headers="headers" :items="properties" pagination="auto" :itemsPerPage="20">


        <template #guestInfo="{item}">
          <v-tooltip bottom>
        ...
        <template #superAdminActions="{ item }">
          <v-icon @click="adminDialog = {show: true, item: item}">mdi-dots-vertical</v-icon>
        </template>
      </ehc-table>
    </ehc-sheet>
    ...
    <ehc-dialog v-model="adminDialog.show" :title="'admin features: ' + adminDialog.item.name" max-width="700px">
      ...
      <v-col cols="4">
        <v-text-field v-if="!newUidMessage" v-model="newUid" label="New User Id"
          ...
          @click:append-outer="transferProperty({ propertyId: adminDialog.item.propertyId, uid: newUid.trim()})">
        </v-text-field>
        <v-checkbox v-if="!newUidMessage" v-model="moveGuestInfo" label="Transfer guest data also" :value=moveGuestInfo
